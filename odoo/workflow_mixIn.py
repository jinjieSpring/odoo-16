...
WORKFLOW_STATE = [('已终止', '已终止'),
                  ('已取消', '已取消'),
                  ('已废弃', '已废弃')]


class WorkFlowMixln(object):
    ...
    def action_confirm(self):
        ...
        finally_main_record_state_desc = ''
        if hasattr(self, 'depend_state'):
            ...
            finally_main_record_state_desc = self.state
        ...
        if process:
            # 筛选process_line
            line = process.process_ids.filtered(lambda o: o.name == finally_main_record_state_desc)
            if len(line) > 1:
                raise UserError(u'警告：系统流程配置错误！')
            else:
                # 取line的下一个节点，因为配置的东西全在这个节点上
                line_next = line.next_id
                # 判断line是不是有跳转
                if line.is_jump:
                    #执行方法
                    globals_dict = {'model': self}
                    line_name = safe_eval.safe_eval(line.jump_code, globals_dict) or line_next.name
                    line_next = process.process_ids.filtered(lambda o: o.name == line_name)
                    copy_line = line
        ...
        if context.get('external_create_order_next_node'):
            return self.env['amos.approval.check.user'].with_context(context).object_ok()
        # 处理审批人和多流程的时候
    ...
    def action_back(self):
        ...
        if workflows[0].state == '等待审批' and workflows[0].name in finally_can_back_node:
                ...
                after_records = self.env['amos.workflow'].sudo().search([('res_model', '=', self._name), ('res_id', '=', self.id), ('id', '>', last_xinjian_record[0].id), ('user_id', '=', False), ('name', '!=', workflows[0].name)])
                if len(after_records) == (finally_can_back_node.index(workflows[0].name)):
                    ...
                    write_records = self.env['amos.workflow'].sudo().search(
                    ...
                    self.env['amos.personnel.process.record'].with_context(active_model=self._name, active_id=self.id).sudo().create({
    ...
    def _compute_workflow_look(self):
        for r in self:
            appr = self.env['amos.personnel.process.record'].sudo().search([('valid', '=', '有效'),
                                                                     ('res_model', '=', self._name),
                                                                     ('res_id', '=', r.id),
                                                                     ('user_id', '=', self._uid)])
            if appr:
    ...
    def action_get_attachment_view(self):
        """附件上传动作视图"""
        self.ensure_one()
        address_form_id = self.env.ref('amos_workflow.view_basic_attachment').id
        return {'type': 'ir.actions.act_window',
                'name': '上传文件',
    ...
    def create_workflow_new(self, record):
        ...
        self.env['amos.personnel.process.record'].with_context({'active_model': record._name, 'active_id': record.id}).sudo().create({
                    'name': '新建' + '---->' + '新建',
    ...
    def get_print_workflow(self, state_desc):
        ...
        return self.env['amos.workflow'].sudo()
    ...
    def close_workflow_to_state(self, depend_state, state, note, workflow_state):
        ...
        self.env['amos.personnel.process.record'].search([('res_model', '=', self._name), ('res_id', '=', self.id), ('valid', '=', '有效')]).write({
            'valid': '无效'
        })
